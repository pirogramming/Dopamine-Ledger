...
def _is_member(user, crew):
    ...
    return CrewMember.objects.filter(crew=crew, user=user).exists()
...
def _process_join(request, crew):
    ...
    # 통과 → 멤버 등록 + 활성 전환
    CrewMember.objects.create(crew=crew, user=request.user)
    crew.refresh_status()
...
def crew_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # 1) 폼에서 크루명만 꺼내되, 아직 저장은 안 함 (commit=False)
            crew = form.save(commit=False)
            # 2) 초대 코드 자동 생성 + status는 모델 기본값(WAITING) 유지
            crew.invite_code = generate_invite_code()
            crew.owner = request.user
            crew.save()
            # 3) 만든 사람을 첫 멤버로 등록 (생성자 = 방장)
            CrewMember.objects.create(crew=crew, user=request.user)
            # 4) 생성 후 상세 화면으로 이동
...
def crew_detail(request, crew_id):
    ...
    members = crew.members.select_related('user')
...
def crew_list(request):
    # 내가 속한 크루만 가져오기 — CrewMember로 연결된 것들
    my_memberships = (
        CrewMember.objects
...
def crew_join_by_link(request, invite_code):
    ...
    # GET = 링크를 눌러 들어온 것 → 확인 화면
    already_member = CrewMember.objects.filter(crew=crew, user=request.user).exists()
...
def crew_leave(request, crew_id):
    ...
    if request.method == 'POST':
        membership = CrewMember.objects.filter(crew=crew, user=request.user).first()
        if not membership:
            messages.info(request, '이미 이 크루의 멤버가 아니에요.')
            return redirect('crew:list')

        was_owner = (crew.owner_id == request.user.id)   # 나가는 사람이 크루장이었나?
        membership.delete()
        ...
        if was_owner:
            # 크루장이 나갔고 남은 사람이 있음 → 가장 오래된 멤버에게 승계
            new_owner_membership = remaining.first()
            crew.owner = new_owner_membership.user
            crew.save(update_fields=['owner'])
            messages.success(
                request,
                f'"{crew.name}" 크루에서 나왔어요. 새 크루장은 {new_owner_membership.user}님이에요.'
            )
        else:
...
def crew_members_api(request, crew_id):
    ...
    members = crew.members.select_related('user').order_by('joined_date')

    data = {
        'status': crew.get_status_display(),      # "활성" / "대기중"
        'member_count': crew.member_count,
        'max_members': crew.MAX_MEMBERS,
        'owner_id': crew.owner_id,                # 크루장 판별용
        'members': [
            {
                'name': str(m.user),
                'is_owner': (m.user_id == crew.owner_id),
